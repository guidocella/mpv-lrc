...
lrc_path=$(echo '{ "command": ["get_property", "path"] }' | socat - /tmp/mpv-socket | jq -r .data)
[ "$lrc_path" ] || exit 1
lrc_path=${lrc_path%.*}.lrc
case $lrc_path in
    /*) ;;
    *) lrc_path=$(echo '{ "command": ["get_property", "working-directory"] }' | socat - /tmp/mpv-socket | jq -r .data)/$lrc_path
esac
...
artist=$(echo $metadata | jq -r 'if has("artist") then .artist else .ARTIST end')
title=$(echo $metadata | jq -r 'if has("title") then .title else .TITLE end')
album=$(echo $metadata | jq -r 'if has("album") then .album else .ALBUM end')
[ "$album" = null ] && album= || album="[album:$album]
"
duration=$(echo '{ "command": ["get_property", "duration"] }' | socat - /tmp/mpv-socket | jq .data)
seconds=${duration%.*}
...
if echo $query | grep -Eiq '([ぁ-ヺ一-龢]|KOTOKO |Ceui )'; then
...
if [ "$WAYLAND_DISPLAY" ] && command -v wl-copy >/dev/null; then
    ...
    echo -n $query | xclip -selection clipboard
fi
